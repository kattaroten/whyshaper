...
public class WhyShaperFlatten extends FlattenTool {
    // gaussian with sig=1
    static float[][] kernel = {
    ...
    public static void apply(Level world, List<BlockPos> targetPositions, Direction facing, Player player) {
        ...
        for (BlockPos p : surfaces) {
            ...
            while (surfaceCoord > targetCoord) {
                ...
                TerrainUtils.placeOrReplace(world, below, blockState, player);
                TerrainUtils.placeOrReplace(world, p, blockState.getFluidState().createLegacyBlock(), player);
            ...
            while (surfaceCoord < targetCoord) {
                ...
                if (!(blockState.getBlock() instanceof LiquidBlock))
                    TerrainUtils.placeOrReplace(world, above, blockState, player);
                TerrainUtils.placeOrReplace(world, p, world.getBlockState(p.relative(facing.getOpposite())), player);
                p = p.relative(facing);
                surfaceCoord++;
                if (timeOut-- <= 0)
                    break;
